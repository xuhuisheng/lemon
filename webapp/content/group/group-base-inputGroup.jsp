...
$(function() {
    $("#orgForm").validate({
        ...
        rules: {
            orgname: {
                remote: {
                    url: 'group-base!checkOrgname.do',
                    data: {
...
<form id="orgForm" method="post" action="group-base!saveGroup.do?operationMode=STORE" class="form-horizontal">
  <input id="org_id" type="hidden" name="groupBaseId" value="${param.groupBaseId}">
  <s:if test="model != null">
  ...
  <div class="control-group">
	...
	<div class="controls">
	  <select id="scope_local_global" name="groupTypeId">
      <s:iterator value="groupTypes" var="item">
	    <option value="${item.id}" ${item.id==model.groupType.id ? 'selected' : ''}>${item.name}</option>
	  </s:iterator>
	  </select>
    </div>
  </div>
  <div class="control-group">
    <label class="control-label" for="org_orgname">负责人</label>
	<div class="controls">
	  <input id="group-base_username" type="text" name="username" value="${username}" size="40" class="text" autocomplete="off">
    </div>
  </div>
  <div class="control-group">
    <div class="controls">
