...
$(function () {
    ...
    var url = '${scopePrefix}/mail/mail-attachment-upload.do';
    $('#fileupload').fileupload({
        ...
        dataType: 'json',
			...
			$('#files').append('<span class="badge">'
				...
				+ '<a href="mail-attachment-download.do?id=' + file.id + '">' + file.name + '</a>'
	...
	$(document).delegate('.icon-remove', 'click', function() {
		var el = $(this);
		var url = 'mail-attachment-removeById.do?id=' + el.attr('title');
...
<form id="mailTemplateForm" method="post" action="mail-template-save.do" class="form-horizontal">
  ...
  <div class="control-group">
	...
	<div class="controls">
	  ...
	  <div id="files" class="files">
        <c:forEach items="${model.mailAttachments}" var="item">
	      <span class="badge">
		    <input type="hidden" name="attachmentIds" value="${item.id}">
			<a href="mail-attachment-download.do?id=${item.id}">${item.name}</a>
