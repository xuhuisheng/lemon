...
<form id="asset-infoForm" method="post" action="sku-info-save.do" class="form-horizontal">
  ...
  <div class="form-group">
    <label class="control-label col-md-1" for="asset-info_address">所属分类</label>
    <div class="col-sm-5">
      <select id="assetInfo_category" name="categoryId" class="form-control">
        <option value=""></option>
        <c:forEach var="item" items="${skuCategories}">
        <option value="${item.id}" ${model.skuCategory.id==item.id ? 'selected' : ''}>${item.name}</option>
        </c:forEach>
      </select>
    </div>
  </div>
  ...
  <div class="form-group">
    <label class="control-label col-md-1" for="asset-info_description">颜色</label>
    <div class="col-sm-5">
      <input id="asset-info_description" type="text" name="color" value="${model.color}" size="40" class="form-control">
    </div>
  </div>
  <div class="form-group">
    <label class="control-label col-md-1" for="asset-info_description">尺码</label>
    <div class="col-sm-5">
      <input id="asset-info_description" type="text" name="size" value="${model.size}" size="40" class="form-control">
    </div>
  </div>
  <%--
  <div class="form-group">
    ...
    <div class="col-sm-5">
      <label class="radio-inline">
        <input id="carInfo_status0" type="radio" name="status" value="active" class="" ${empty model || model.status == 'active' ? 'checked' : ''}> 启用
      </label>
      <label class="radio-inline">
        <input id="carInfo_status1" type="radio" name="status" value="disable" class="" ${model.status == 'disable' ? 'checked' : ''}> 禁用
      </label>
    </div>
  </div>
  --%>
