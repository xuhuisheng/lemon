...
<div class="panel panel-default">
  ...
  <div class="panel-body">
    ...
    <form name="store-infoForm" method="post" action="upload.do" class="form-inline" enctype="multipart/form-data">
      <input type="hidden" name="bucket" value="${param.bucket}">
      <input type="file" name="file" class="file" style="display:inline-block;vertical-align:middle;">
  ...
  <table id="storeInfoGrid" class="table table-hover">
    ...
    <tbody>
      ...
      <tr>
        ...
        <td><a href="${tenantPrefix}/oss/rs/${param.bucket}/${item.name}">${item.name}</a></td>
        <td>${item.createTime}</td>
        <td>
          <a href="remove.do?id=${item.id}" class="a-update">删除</a>
        </td>
      </tr>
      </c:forEach>
    </tbody>
