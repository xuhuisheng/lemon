...
ProcessDiagramCanvas.prototype = {
	...
	_drawFlow : function(waypoints, conditional, isDefault, highLighted,
			withArrowHead, connectionType, flow) {
		var originalPaint = this.getPaint();
		...
		if (withArrowHead)
			arrowHead = this._drawArrowHead(line, connectionType, flow,
					newWaypoints);
		...
		polyClone.click(function() {
			...
			var contextObject = object.data("contextObject");
			if (contextObject)
				ProcessDiagramGenerator.showFlowInfo(contextObject, mouseEvent);
		});
	...
	drawSequenceflow : function(waypoints, conditional, isDefault, highLighted,
		...
		this._drawFlow(waypoints, conditional, isDefault, highLighted,
				withArrowHead, CONNECTION_TYPE.SEQUENCE_FLOW, flow);
	},
	 ...
	 * @return {}
	 */
	_drawArrowHead : function(line, connectionType, flow, newWaypoints) {
		...
		// add by houyongbo 20140228 begin
		if (window.navigator.userAgent.indexOf("MSIE") >= 1) {
			// 用旋转的角度来判断是不是向上的箭头（向下是0，顺时针是正值，逆时针是负值）
			if (parseInt(Raphael.deg(line.angle - Math.PI / 2)) < -170) {
				line.x2 -= 3;
			}
			line.y2 -= 3;
		}
		...
		for (var i = 0; i < newWaypoints.length; i++) {
			if (i < newWaypoints.length - 1) {
				// 计算两个坐标点之间的距离
				distince += Math.sqrt(Math.pow(newWaypoints[i].x
								- newWaypoints[i + 1].x, 2)
						+ Math
								.pow(newWaypoints[i].y - newWaypoints[i + 1].y,
										2))
						+ ",";
			}
		}
		distince = distince.substring(0, distince.length - 1);
		var distinceArray = distince.split(",");

		// 去数组中最大的距离
		var maxDic = Math.max.apply(Math, distinceArray);

		// 获取数据做大值的下标
		var idx = 0;
		for (var i = 0; i < distinceArray.length; i++) {
			if (distinceArray[i] == maxDic) {
				idx = i;
			}
		}
		...
		if (flow.name != null) {
			// 判断是水平文字还是垂直文字
			var angle = Math.abs(newWaypoints[idx].y - newWaypoints[idx + 1].y) > Math
					.abs(newWaypoints[idx].x - newWaypoints[idx + 1].x)
					? 270
					: 0;
			var textLen = flow.name.replace(/[^\x00-\xff]/g, '.').length * 20;
			var _x, _y;

			// 两个坐标点中间点的x轴
			var _xDis = newWaypoints[idx].x > newWaypoints[idx + 1].x
					? newWaypoints[idx + 1].x
							+ (newWaypoints[idx].x - newWaypoints[idx + 1].x)
							/ 2
					: newWaypoints[idx].x
							+ (newWaypoints[idx + 1].x - newWaypoints[idx].x)
							/ 2;

			// 两个坐标点中间点的y轴
			var _yDis = newWaypoints[idx].y > newWaypoints[idx + 1].y
					? newWaypoints[idx + 1].y
							+ (newWaypoints[idx].y - newWaypoints[idx + 1].y)
							/ 2
					: newWaypoints[idx].y
							+ (newWaypoints[idx + 1].y - newWaypoints[idx].y)
							/ 2;
			if (angle == 0) {
				_x = _xDis - textLen / 2 - 10;
				_y = _yDis - 15;// 放在分支线的上方
				this.drawFlowLabel(flow.name, _x, _y, textLen, textLen, true,
						angle, MULTILINE_VERTICAL_ALIGN_TOP,
						MULTILINE_HORIZONTAL_ALIGN_MIDDLE);
			}

			if (angle == 270) {
				_x = _xDis - textLen / 2 - 13;
				_y = _yDis - 10;// 放在分支线的上方
				this.drawFlowLabel(flow.name, _x, _y, textLen + 10, textLen
								+ 10, true, angle,
						MULTILINE_VERTICAL_ALIGN_MIDDLE,
						MULTILINE_HORIZONTAL_ALIGN_RIGHT);
			}
		}
		// add by houyongbo 20140228 end
	...
	_drawTask : function(name, x, y, width, height, thickBorder) {
		...
		if (name) {
			...
			this.drawTaskLabel(name, boxX, boxY, boxWidth, boxHeight, false);
		}
	},
	...
	drawTaskLabel : function(text, x, y, boxWidth, boxHeight) {
		...
		this
				._drawMultilineText(text, x, y, boxWidth, boxHeight,
	 ...
	 *            angle 旋转角度
	 */
	drawFlowLabel : function(text, x, y, boxWidth, boxHeight, needRotate,
			angle, verticalAlign, horizontalAlign) {
		var originalFont = this.getFont();
		this.setFont(TASK_FONT);

		this._drawMultilineText(text, x, y, boxWidth, boxHeight, verticalAlign,
				horizontalAlign, needRotate, angle);
	...
	drawMultiInstanceMarker : function(sequential, x, y, width, height) {
		...
		if (sequential) {
			var line = this.g.path("M" + lineX + " " + lineY + "L"
					+ (lineX + rectangleWidth) + " " + lineY + "M" + lineX
	...
	_drawMultilineText : function(text, x, y, boxWidth, boxHeight,
			verticalAlign, horizontalAlign, needRotate, angle) {
		if (!text || text == "")
			return;
		...
		// add by houyongbo 20140303 begin 因为垂直居中不能生效，所以只能这用那个设置
		if (verticalAlign == "middle" && needRotate) {
			measurer.rafaelTextObject.attr({
						"y" : y + height / 2
					});
		}
		// add by houyongbo 20140303 end
		// var bb = measurer.rafaelTextObject.getBBox();
		...
		// houyongbo comment：这里放开hide方法就可以显式label的边框
